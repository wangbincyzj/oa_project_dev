  ...
  <div class="hfxsqr">
    <ContainerTwoType :nav-info="navInfo" @liClick="liClick">
      <TitleTable title="合法销售确认-选择房间">
        ...
        <RoomsUnit ref="roomStructure" @roomClick="roomClick"/>
      </TitleTable>
    </ContainerTwoType>
    <el-dialog
        title="签订承诺书-确认房间"
...
export default {
  name: "Hfxsqr2",
  components: {RoomsUnit, BuildingStructure, RoomColor, HfxsqrDialog, RoomStructure, TitleTable, MyNav, ContainerTwoType},
  ...
  created() {
    ...
    console.log(this.$store.state.buildingInfo)
  ...
  methods: {
    liClick(index) {
      ...
      this.$refs.roomStructure.fetchRooms(this.navInfo.list[index].id)
    },
    roomClick(room) {
      if (room.roomFwzt === 2) {
    ...
    submitSuccess() {
      ...
      this.$refs.roomStructure.fetchRooms(this.ldxxId)
    }
