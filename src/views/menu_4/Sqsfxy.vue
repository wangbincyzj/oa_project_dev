  ...
  export default {
    ...
    methods:{
      fetchNavInfo() {
        ...
        sqjgzhApi.getAllAccount(1, 100).then(ret => {
           this.navInfo.list = ret.data.records.map(item=>({
            ...item, id: item.zjjgzhId, name: item.zjjgzhZhmc
          }));
          this.navInfo.list.unshift({id:0, name: "请选择监管账户名称"});
        })
      },
      fetchData(){
       sqsfxyApi.getAllContract(this.currentPage, this.pageSize,this.$store.state.rwbh).then(ret => {
     ...
     fetchDataByAccountId(id){
       ...
       sqsfxyApi.getContractByAccount(this.currentPage, this.pageSize,this.$store.state.rwbh,id).then(ret => {
         console.log(ret);
